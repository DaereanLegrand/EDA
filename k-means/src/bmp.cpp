...
void
write_modified(img image, char *outputname, char *k)
{
    char *fname = (char*)malloc(sizeof(char) * (strlen(outputname) + strlen(k) + 15));
    fname[0] = '\0';
    strcat(fname, "kmeans-");
    strcat(fname, k);
    strcat(fname, "-");
    strcat(fname, outputname);
    // outputname + k + ".bmp";
    ...
    fclose(res);
    char *callsxiv = (char*)malloc(sizeof(char) * (strlen(fname) + 10));
    callsxiv[0] = '\0';
    strcat(callsxiv, "sxiv ");
    strcat(callsxiv, fname);
    system(callsxiv);
}
